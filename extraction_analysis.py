...
def plotting_overlaid(chD, plotTitle, xLabel, yLabel, showing):
    fig = plt.figure()
    for i in range(len(chD)):
        plt.plot(chD[i],alpha=0.1)
    plt.xlabel(xLabel)
    plt.ylabel(yLabel)
    plt.title(plotTitle)
    if showing ==  0:
...
def moving_average_2(chD, n):
    cumsum = np.cumsum(chD, axis=-1)
    newChD = (cumsum[:,n:] - cumsum[:,:-n]) / (2 * n + 1)
    return newChD
...
for inputFile in inputFileList: 
    ...
    ch = 0
    # plotting_overlaid(data[0], 'Overlaid Wfs ch A', 'binned time', 'voltage [mV]', showing)
    # plotting_overlaid(data[1], 'Overlaid Wfs ch B', 'binned time', 'voltage [mV]', showing)
    # plt.show()
    for chData in data:
        ...
        maChData = moving_average_2(chData, 10)
        minimumMaChData = np.min(maChData, axis=-1)
        indexMinMaChData = []
        indexMinNewChData = []
